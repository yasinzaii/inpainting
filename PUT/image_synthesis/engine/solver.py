...
class Solver(object):
    def __init__(self, config, args, model, dataloader, logger):
        ...
        if self.max_epochs < 0:
            self.max_epochs = (self.max_iterations + self.dataloader['train_iterations'] - 1) // self.dataloader[
                'train_iterations']
        ...
        if self.args.distributed:
            ...
            self.model = torch.nn.parallel.DistributedDataParallel(self.model, device_ids=[self.args.gpu],
                                                                   find_unused_parameters=config['solver'].get(
        ...
        self.logger.log_info("{}: global rank {}: prepare solver done!".format(self.args.name, self.args.global_rank),
                             check_primary=False)
    ...
    def _get_optimizer_and_scheduler(self, op_sc_list):
        ...
        for op_sc_cfg in op_sc_list:
            ...
            assert isinstance(op_sc['optimizer']['step_iteration'],
            ...
            if 'scheduler' in op_sc_cfg:
                ...
                if sc_cfg['target'].split('.')[-1] in ['CosineAnnealingLRWithWarmup', 'CosineAnnealingLR']:
                    T_max = self.max_epochs * self.dataloader[
                        'train_iterations'] if self.max_iterations < 0 else self.max_iterations
    ...
    def sample(self, batch, phase='train', step_type='iteration'):
        ...
        with torch.no_grad():
            ...
            for k, v in samples.items():
                ...
                save_path = os.path.join(save_dir, 'e{:010d}_itr{:010d}_rank{}{}'.format(self.last_epoch,
                                                                                         self.last_iter %
                                                                                         self.dataloader[
                                                                                             'train_iterations'],
                                                                                         get_rank(), suffix))
                if torch.is_tensor(v) and v.dim() == 4 and v.shape[1] in [1, 3]:  # image
                    ...
                    self.logger.add_images(tag='{}/{}e_{}itr/{}'.format(phase, self.last_epoch,
                                                                        self.last_iter % self.dataloader[
                                                                            'train_iterations'], k), img_tensor=im,
    ...
    def step(self, batch, phase='train'):
        ...
        for op_sc_n, op_sc in self.optimizer_and_scheduler.items():
            ...
            input = {
                ...
                'return_loss': True,
                'step': self.last_iter,
                'total_steps': self.max_epochs * self.dataloader[
                    'train_iterations'] if self.max_iterations < 0 else self.max_iterations,
            }
            if op_sc_n != 'none':
                input['name'] = op_sc_n
            ...
            if phase == 'train':
                if op_sc['optimizer']['step_iteration'] > 0 and (self.last_iter + 1) % op_sc['optimizer'][
                    'step_iteration'] == 0:
                ...
                if 'scheduler' in op_sc:
                    if op_sc['scheduler']['step_iteration'] > 0 and (self.last_iter + 1) % op_sc['scheduler'][
                        'step_iteration'] == 0:
            ...
            if save or force:
                ...
                if save:
                    save_path = os.path.join(self.ckpt_dir,
                                             '{}e_{}iter.pth'.format(str(self.last_epoch).zfill(6), self.last_iter))
            ...
            if load_others:
                ...
                if self.last_iter != state_dict['last_iter']:
                    self.logger.log_info(
                        'The last interations {} is not the same with state dict {}'.format(self.last_iter,
                                                                                            state_dict['last_iter']))
        ...
        for itr, batch in enumerate(self.dataloader['train_loader']):
            ...
            if self.logger is not None and self.last_iter % self.args.log_frequency == 0:
                ...
                if self.max_iterations > 0:
                    info = info + ': Iter {}/{} epoch {} iter {}/{}'.format(self.last_iter, self.max_iterations,
                                                                            self.last_epoch,
                                                                            self.last_iter % self.dataloader[
                                                                                'train_iterations'],
                ...
                for loss_n, loss_dict in loss.items():
                    ...
                    for k in loss_dict:
                        ...
                        self.logger.add_scalar(tag='train/{}/{}'.format(loss_n, k), scalar_value=float(loss_dict[k]),
        ...
        if val:
            ...
            for itr, batch in enumerate(self.dataloader['validation_loader']):
                ...
                if self.logger is not None and (itr + 1) % self.args.log_frequency == 0:
                    ...
                    if self.max_iterations > 0:
                        info = info + ': Iter {}/{} | epoch {} iter {}/{}'.format(self.last_iter, self.max_iterations,
                                                                                  self.last_epoch, itr, self.dataloader[
                                                                                      'validation_iterations'])
                    else:
                        info = info + ': Epoch {}/{} | iter {}/{}'.format(self.last_epoch, self.max_epochs, itr,
            ...
            if self.logger is not None:
                ...
                for loss_n, loss_dict in overall_loss.items():
                    ...
                    for k in loss_dict:
                        info += ' | {}: {:.4f}'.format(k, float(loss_dict[k]))
                        self.logger.add_scalar(tag='val/{}/{}'.format(loss_n, k), scalar_value=float(loss_dict[k]),
    ...
    def log_validation(self, save_dir):
        ...
        for itr, batch in enumerate(self.dataloader['viz_loader']):
            if itr in list:
                ...
                flow_inpainted = Repeat_Transform().image_to_flow(img1_inpainted, img2_inpainted,
                                                                  max_flow)  #256*256*2 npy
    ...
    def pvqvae_log_validation(self,save_dir):
        ...
        os.makedirs(save_dir + "/img1", exist_ok=True)
        os.makedirs(save_dir + "/img2", exist_ok=True)
        os.makedirs(save_dir + "/img1_res", exist_ok=True)
        os.makedirs(save_dir + "/img2_res", exist_ok=True)
        os.makedirs(save_dir + "/flow_gt", exist_ok=True)
        os.makedirs(save_dir + "/flow_res", exist_ok=True)
        os.makedirs(save_dir + "/flow_gt_viz", exist_ok=True)
        os.makedirs(save_dir + "/flow_res_viz", exist_ok=True)
        for itr, batch in enumerate(self.dataloader['viz_loader']):
            if itr in list:
                index = image_relative_paths[itr]
                img1 = batch['image'].numpy()
                img1 = np.squeeze(img1, axis=0)
                img1 = img1.transpose(1, 2, 0)
                img2 = batch['image2'].numpy()
                img2 = np.squeeze(img2, axis=0)
                img2 = img2.transpose(1, 2, 0)
                output1 = self.model(batch, name="reconstruction")
                img1_res= output1['reconstruction'].cpu().numpy()
                img1_res = np.squeeze(img1_res, axis=0)
                img1_res = img1_res.transpose(1, 2, 0)
                batch['image'] = batch['image2']
                output2 = self.model(batch, name="reconstruction")
                img2_res = output2['reconstruction'].cpu().numpy()
                img2_res = np.squeeze(img2_res, axis=0)
                img2_res = img2_res.transpose(1, 2, 0)
                max_flow = batch['max_flow'].numpy().item()
                flow_gt = Repeat_Transform().image_to_flow(img1, img2, max_flow)
                flow_res = Repeat_Transform().image_to_flow(img1_res, img2_res, max_flow)
                flow_gt_viz = flow_to_image(flow_gt)
                flow_res_viz = flow_to_image(flow_res)
                save_img1 = save_dir + "/img1/" + index + ".npy"
                save_img2 = save_dir + "/img2/" + index + ".npy"
                save_img1_res = save_dir + "/img1_res/" + index + ".npy"
                save_img2_res = save_dir + "/img2_res/" + index + ".npy"
                save_flow_gt = save_dir + "/flow_gt/" + index + ".npy"
                save_flow_res = save_dir + "/flow_res/" + index + ".npy"
                save_flow_gt_viz = save_dir + "/flow_gt_viz/" + index + ".png"
                save_flow_res_viz = save_dir + "/flow_res_viz/" + index + ".png"
                np.save(save_img1, img1)
                np.save(save_img2, img2)
                np.save(save_img1_res, img1_res)
                np.save(save_img2_res, img2_res)
                np.save(save_flow_gt, flow_gt)
                np.save(save_flow_res, flow_res)
                flow_gt_viz = Image.fromarray(flow_gt_viz)
                flow_res_viz = Image.fromarray(flow_res_viz)
                flow_gt_viz.save(save_flow_gt_viz)
                flow_res_viz.save(save_flow_res_viz)
