...
def numpy_to_png(numpy_array, output_path):
    # Ensure the numpy array has the shape (H, W, 1)
    if len(numpy_array.shape) != 3 or numpy_array.shape[2] != 1:
        raise ValueError("Input numpy array must have shape (H, W, 1)")

    # Convert the numpy array to a format suitable for saving as a PNG
    # Scale the values to 0-255 if they are not already
    ...
    cv2.imwrite(output_path, numpy_array)
...
if __name__ == '__main__':
    ...
    print(mask)
    output_path='/data/zhujinxian/code/put/PUT/data/mask_sample/mask.png'
    numpy_to_png(mask, output_path)
