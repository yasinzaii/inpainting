...
from put.PUT.image_synthesis.utils.io import load_yaml_config, load_dict_from_json, save_dict_to_json
from put.PUT.image_synthesis.utils.misc import get_all_file, get_all_subdir, instantiate_from_config
from put.PUT.image_synthesis.utils.cal_metrics import get_PSNR, get_mse_loss, get_l1_loss, get_SSIM, get_mae
from put.PUT.image_synthesis.modeling.build import build_model
from put.PUT.image_synthesis.utils.misc import format_seconds, merge_opts_to_config
...
class ImagePathDataset(torch.utils.data.Dataset):
    ...
    def read_image(self, image_path):
        ...
        flow_numpy=np.load(image_path)
        flow_numpy=self.crop_image(flow_numpy)
        image,max_flow=Normalize_Transform().flow_to_image(flow_numpy)
        ...
        return image_inp, image,max_flow,flow_numpy
    ...
    def __getitem__(self, i):
        ...
        image_inp, image,max_flow,flow_numpy = self.read_image(os.path.join(self.image_dir, image_path))
...
def inference_inpainting(local_rank=0, args=None):
    ...
    # save images
    save_root =cwd+ '/RESULTS/resize_occlusion_image_inpainted'
    # if args.save_dir:
    ...
    for fr in filter_ratio:
        for ntp in num_token_per_iter:
            ...
            for i in range(start_index, end_index):
                ...
                # import pdb; pdb.set_trace()
                max_flow=data_i['max_flow']
                flow_numpy=data_i['flow_numpy']
                # save masked image
                #test
                args.save_masked_image=True
                if args.save_masked_image:
                    mask_=(data_i['mask'][0]).permute(1, 2, 0).to('cpu').numpy()
                    ...
                    mask_im_ = Image.fromarray(mask_im.astype(np.uint8))
                    mask_= mask_.squeeze(axis=2)
                    mask_=(mask_.astype(np.uint8))*255
                    mask_=Image.fromarray(mask_,mode='L')
                    if args.num_sample > 1:
                        save_path = os.path.join(save_root_, 'gt_mask_image.png')
                    else:
                        # import pdb; pdb.set_trace()
                        save_path_mask_image = os.path.join(save_root_, 'mask_image')
                        save_path_mask_image = save_path_mask_image + '/' + index + '.png'
                        print('save mask_image in'+save_path_mask_image)
                    ...
                    # import pdb; pdb.set_trace()
                    mask_im_.save(save_path_mask_image)
                ...
                while count_per_cond_ < args.num_sample:
                    ...
                    for k in content_dict.keys():
                        ...
                        if k in ['completed']:
                            ...
                            save_path_u = os.path.join(save_root_, 'diff_u')
                            save_path_u = save_path_u + '/' + index + '.png'
                            print('save diff_u in' + save_path_u)
                            save_path_v = os.path.join(save_root_, 'diff_v')
                            save_path_v = save_path_v + '/' + index + '.png'
                            print('save diff_v in' + save_path_v)
                            content_flow = cv2.resize(content_flow, tuple((256,256)), interpolation=cv2.INTER_NEAREST)
                            flow_numpy = cv2.resize(flow_numpy, tuple((256,256)), interpolation=cv2.INTER_NEAREST)
                            import draw_diff
                            draw_diff.draw_diff(ori_flow=flow_numpy,generate_flow=content_flow,save_path_u=save_path_u,save_path_v=save_path_v)
                            #上述为修改加入的代码
